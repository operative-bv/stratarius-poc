...
create table public.param_plafond (
    ...
    jaarplafond numeric(18, 4) not null check (jaarplafond >= 0),
    kwartaalplafond numeric(18, 4) null check (kwartaalplafond is null or kwartaalplafond >= 0),
...
create table public.param_rsz (
    ...
    geldig_van date not null,
    geldig_tot date null,
    basisbijdrage_pct numeric(6, 4) not null check (basisbijdrage_pct between 0 and 1),
    basisfactor_pct numeric(6, 4) null check (basisfactor_pct is null or basisfactor_pct between 1 and 2),
    bron_url text not null,
    bron_document text null,
    created_at timestamptz not null default now(),
    updated_at timestamptz not null default now(),
